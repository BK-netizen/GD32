#include "driver_malloc.h"	   

// 内存池(64字节对齐)
SET_AXI uint8_t mem1base[MEM1_MAX_SIZE];					// 内部SRAM内存池
SET_SDRAM   uint8_t mem2base[MEM2_MAX_SIZE];					// 外部SDRAM内存池
//__align(32) uint8_t mem3base[MEM3_MAX_SIZE] __attribute__((at(0X10000000)));					// 内部CCM内存池

// 内存管理表                                                                               	   
SET_AXI uint32_t mem1mapbase[MEM1_ALLOC_TABLE_SIZE];													// 内部SRAM内存池MAP
SET_SDRAM uint32_t mem2mapbase[MEM2_ALLOC_TABLE_SIZE];		// 外部SDRAM内存池MAP
